...
function page() {
    ...
    useEffect(() => {
        const getAsg = async (id) => {
            const res = await axios.post('/api/get_all_asg', { classId: id })
            if (res.status === 200) {
                const data = res.data
                setAssigment(data.assigments)

                console.log(data.assigments)
                setLoading(false)
            }
        }
        getAsg(id)
        ...
        <div>
            ...
            <ul>
                {assigments.length > 0 && assigments.map((assignment) => (
                    <li key={assignment._id} className="mb-4 p-4 bg-gray-100 rounded-lg shadow-md">
                        <div>
                            <p className="font-semibold text-indigo-600">
                                <Link
                                    href={`/asg/${assignment._id}`}
                                    target="_blank"
